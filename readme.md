...
3. Нажать правой кнопкой мыши на иконку "Обновить страницу"
4. Выбрать пункт "Очистка кеша и жесткая перезагрузка". 
<br/>
Так вы получите актуальную версию проекта. Изменения в проекте могут отображаться с задержкой, поэтому на всякий случай лучше сделать так.
<br/>
<br/>
Тестовые чеки, на которых мы проверяли работу приложения, находятся в папке "Тестовые чеки".

**Для проверки сканирования QR кода нужен любой чек из магазина, на нем будет QR код (сканировать желательно с телефона, с веб-камеры может не хватать качества)**
<br/>
...
- **OCR.Space** - бесплатный сервис с хорошими результатами распознавания.
...
*ВАЖНО - наше приложение получает доступ ТОЛЬКО К ПУБЛИЧНОЙ информации на вашей странице ВК. Чисто технически невозможно получить дополнительную информацию или совершать какие-либо действия. Отвязать аккаунт можно на сайте ВК*
 ...
 **Деление чека**<br/>
1. **Пользователь загружает чек в приложение ИЛИ сканирует QR-код**
...
3. **Пользователь выбирает людей, которые ели с ним ИЛИ отправляет ссылку друзьям**.  Проверить совместное деление можно зайдя через режим "Инкогнито" в браузере либо с другого устройства.
4. **Пользователь указывает список позиций для каждого человека ИЛИ только свои позиции** (включая количество порций). 
...
Чеки, которые тестировались при работе находятся в папке "Тестовые чеки". Для тестирования QR-кодов подойдет любой чек из любого магазина.
